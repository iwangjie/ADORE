...
/**
 * Created by adore on 15/11/6.
 * 账外资产盘点
 */
public class UnlistedAssetsWorkflowAction implements Action{
    BaseBean log = new BaseBean();//定义写入日志的对象

    public String execute(RequestInfo info) {

        log.writeLog("新增盘点UnlistedAssetsWorkflowAction——————");
        ...
        sql = " insert into formtable_main_90_dt1(mainid,assid,pdr,plant,pdType) "
                +" select "+requestid+",id,"+empid+","+plant1+","+type+" from formtable_main_88 where formmodeid = 32 and status = 0 ";
        rs.execute(sql);

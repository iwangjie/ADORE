...
public class BringMainAndDetailByMain {
    ...
    public static String getFunctionNameByWorkflowID(String id) {
        String result = "";
        String sql = "select distinct SAPFunctionName from OA_SAP_MAPPING where workflowId ="
                + id;
    ...
    public String getReturn(Map<String, String> oaDatas, String workflowID) {
        ...
        try {
            ...
            basebean.writeLog("SAP函数名：" + functionName);
            ...
            basebean.writeLog("传进来字段键值对" + oaDatas.toString());
            Set<String> keys = oaDatas.keySet();
            Iterator<String> it = keys.iterator();
            while (it.hasNext()) {
                String oaFieldName = it.next();
                basebean.writeLog("OA字段的名称为：" + oaFieldName);
                String value = oaDatas.get(oaFieldName);
                basebean.writeLog("OA字段的值为：" + value);
                try {
                    ...
                    if ("inPara".equals(bean.getSAPParameterType())) {
                        basebean.writeLog("输入参数的名称为：" + bean.getSAPParameterName());
            ...
            for (SapOaMappingBean outBean : otherlist) {
                ...
                basebean.writeLog("获取结果 ：key=" + outValue + " ; value = " + outBean.getSAPFieldName());
            ...
            basebean.writeLog("最后从SAP返回的JSON数组" + jsonObj.toString());
	...
	public static String getPageIDByOAFieldDt(String formid, String fieldName) {
		String sql = "select id from workflow_billfield where billid = "
				+ formid + " and fieldname ='" + fieldName
				+ "' and viewtype = 1 ";
		basebean.writeLog("获取页面明细sql：" + sql);
		RecordSet rs = new RecordSet();
		rs.execute(sql);
		String id = "";
		if (rs.next()) {
    ...
    public static List<SapOaMappingBean> getOutParameters(String workflowId,
        ...
        String sql = "select *  from OA_SAP_MAPPING where workflowid = "
                + workflowId + " and OATableType like '" + OAFieldType
                + "%'and  SAPParameterType like 'out%'";
        basebean.writeLog("获取输出的相关信息sql：" + sql);
        RecordSet rs = new RecordSet();
        rs.execute(sql);
        while (rs.next()) {
    ...
    public static SapOaMappingBean getInputParameter(String workflowId,
        ...
        String sql = "select *  from OA_SAP_MAPPING where OAFieldName ='"
    ...
    public static List<String> getOAParaName(String workflowId) {
        ...
        String sql = "select OAFieldName from OA_SAP_MAPPING where workflowId = "
    ...
    public static List<SapOaMappingBean> getRelation(String workflowId) {
        String sql = "select OAFieldName, SAPParameterType,SAPParameterName,SAPFieldName from OA_SAP_MAPPING where workflowId = "
                + workflowId;
        sql = "select * from OA_SAP_MAPPING where workflowId = " + workflowId;
     ...
     * @return
     */
    private static JCO.Client getSAPcon() {
        // 得到数据源的连接

...
/**
 * Created by adore on 16/8/9.
 */
public class RecoveryOutPlanWorkflowAction implements Action{
    public String execute(RequestInfo info) {

        BaseBean log = new BaseBean();

        log.writeLog("进入外出计划恢复RecoveryOutPlanWorkflowAction");
        ...
        if (!"".equals(tableName)) {
            ...
            if (rs.next()) {
                ...
                if (!"".equals(relatedapply)) {
                    sql = " update uf_Replace_table set isActive =0,modedatacreatedate = CONVERT(varchar(10) ,GETDATE(), 23 ) ,"
                            + " modedatacreatetime = CONVERT(varchar(5) ,GETDATE(), 114 ) where requestId= " + relatedapply + "  ";
                    res.executeSql(sql);
                } else {
                    log.writeLog("需要恢复的信息获取失败!");
                    return "-1";
                }
            }
        } else {
